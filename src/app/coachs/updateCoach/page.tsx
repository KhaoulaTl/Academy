...
import { getAllCoachesThunk, updateCoachThunk } from "@/lib/services/coach/coach";
...
interface UpdateCoachProps {
  ...
  const UpdateCoach = ({ coach, onClose }: UpdateCoachProps) => {
    if (!coach) return null; // or return a loading indicator, etc.
  ...
  const formMethods = useForm<UpdateCoachFormData>({
    defaultValues: {
      _id: coach._id,
      firstName: coach.firstName,
      lastName: coach.lastName,
      phone: coach.phone,
      ageCategory: coach.ageCategory,
      playerIds: coach.playerIds,
    },
  });
  ...
  }, [coaches, resetUpdateCoachForm]);

  const onSubmitCoach: SubmitHandler<UpdateCoachFormData> = async (data) => {
    const updatedCoach = { ...data };
    setIsLoading(true);
  
    try {
      const response = await dispatch(updateCoachThunk({ id: coach._id, requestData: updatedCoach }));
      if (response.meta.requestStatus === "fulfilled") {
        setAlertMessage("Les détails d'entraîneur ont été mise à jour avec succès.");
        setSuccessShowAlert(true);
        setIsLoading(false);
        // Réinitialiser les valeurs du formulaire avec les données mises à jour
        reset({
          firstName: response.payload.firstName,
          lastName: response.payload.lastName,
          phone: response.payload.phone,
          ageCategory: response.payload.ageCategory,
          playerIds: response.payload.playerIds,
        });

              // Mettre à jour l'état des entraîneurs
              setCoaches((prev: any[]) => {
                const updatedCoaches = prev.map((coach: any) => {
                  if (coach._id === response.payload._id) {
              ...
              <div className="mb-5 rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
                  <div className="border-b border-stroke px-6.5 py-4 dark:border-strokedark">
                      <h3 className="font-medium text-black dark:text-white">
                          Mettre à jour les détails d'un entraîneur
                      </h3>
                  </div>

                  <div className="p-7">
                    <FormProvider {...formMethods}>
                          <form onSubmit={handleUpdateCoach(onSubmitCoach)}>
                              ...
                              <div className="flex justify-end gap-4.5">
                                  ...
                                  <button
                                      ...
                                      disabled={isLoading}
                                  >
                                      {isLoading ? "Chargement..." : "Mettre à jour les détails d'entraîneur"}
                                  </button>
                                  <button
