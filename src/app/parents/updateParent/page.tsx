...
const UpdateParent = ({ parent, onClose }: UpdateParentProps) => {
    ...
    const formMethods = useForm<UpdateParentFormData>({
        defaultValues: {
          _id: parent._id,
    ...
    const fetchParents = useCallback(async () => {
      ...
      }, [parents, resetUpdateParentForm]);

    const onSubmitParent: SubmitHandler<UpdateParentFormData> = async (data) => {
        const updatedParent = { ...data };
        setIsLoading(true);
      
        try {
          const response = await dispatch(updateParentThunk({ id: parent._id, requestData: updatedParent }));
          if (response.meta.requestStatus === "fulfilled") {
            setAlertMessage("Les détails de parent ont été mise à jour avec succès.");
            ...
            reset({
              ...
              phone1: response.payload.phone1,
              phone2: response.payload.phone2,
              childIds: response.payload.childIds,
            });
    
                  // Mettre à jour l'état des parents
                  setParents((prev: any[]) => {
                    const updatedParents = prev.map((parent: any) => {
                      if (parent._id === response.payload._id) {
            ...
            router.push(setting.routes.Parents);
            onClose();
          } else {
        ...
        <div className="mb-5 rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
            ...
            <div className="border-b border-stroke px-6.5 py-4 dark:border-strokedark">
                <h3 className="font-medium text-black dark:text-white">
                    Mettre à jour les détails de parent
                </h3>
            </div>

            <div className="p-7">
            <FormProvider {...formMethods}>
                <form onSubmit={handleUpdateParent(onSubmitParent)}>
                    ...
                    <p className="text-red-500 text-sm">{errors.phone2.message}</p>
                            ...
                            <div className="flex justify-end gap-4.5">
                                  ...
                                  <button
                                      ...
                                      disabled={isLoading}
                                  >
                                      {isLoading ? "Chargement..." : "Mettre à jour les détails de parent"}
                                  </button>
                                  <button
