...
import { getAllPlayersThunk, updatePlayerThunk } from '@/lib/services/player/player';
...
interface UpdatePlayerProps {
  ...
  const UpdatePlayer = ({ player, onClose }: UpdatePlayerProps) => {
    ...
    const formMethods = useForm<UpdatePlayerFormData>({
        defaultValues: {
          _id: player._id,
    ...
    }, [players, resetUpdatePlayerForm]);


    const onSubmitPlayer: SubmitHandler<UpdatePlayerFormData> = async (data) => {
        const updatePlayer = { ...data };
        setIsLoading(true);

        try {
            const response = await dispatch(updatePlayerThunk({ id: player._id, requestData: updatePlayer }));
            if (response.meta.requestStatus === "fulfilled") {
                setAlertMessage("Les détails de footballeur ont été mise à jour avec succès.");
                setSuccessShowAlert(true);
                setIsLoading(false);
                reset({
                    firstName: response.payload.firstName,
                    lastName: response.payload.lastName,
                    birthDate: response.payload.birthDate,
                    parentId: response.payload.parentId,
                    coachId: response.payload.coachId,
                    skillLevel: response.payload.skillLevel
                });

                setPlayers((prev: any[]) => {
                    const updatePlayer = prev.map((player: any) => {
                        if (player._id === response.payload._id) {
            ...
            setAlertMessage("Erreur de mise à jour des données");
...
// Function to handle changes for parent selection
const handleParentChange = (value: string) => {
    reset((prev) => ({ ...prev, parentId: value })); // Update form state
};

// Function to handle changes for coach selection
const handleCoachChange = (value: string) => {
    reset((prev) => ({ ...prev, coachId: value })); // Update form state
};
    ...
    const parents = parentDetails && parentDetails.map((parent: { _id: any; firstName: any; lastName: any; }) => ({
      ...
      <div className="border-b border-stroke px-6.5 py-4 dark:border-strokedark">
                <h3 className="font-medium text-black dark:text-white">
                    Mettre à jour les détails de footballeur
                </h3>
            </div>

            <div className="p-7">
            <FormProvider {...formMethods}>
            <form onSubmit={handleUpdatePlayer(onSubmitPlayer)}>
...
<button
    ...
    disabled={isLoading}
>
    {isLoading ? "Chargement..." : "Mettre à jour les détails de footballeur"}
</button>
<button
